...
RUN cd /app/git && \
    ...
    wget -O /usr/local/share/tar1090/aircraft.csv.gz https://github.com/katlol/tar1090-db/releases/latest/download/aircraft.csv.gz

FROM debian:bookworm-slim
RUN apt-get update && \
    apt-get -y install --no-install-recommends libusb-1.0-0-dev librtlsdr-dev librtlsdr0 libncurses-dev zlib1g-dev zlib1g libzstd-dev libzstd1 && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
ENV LD_PRELOAD=/usr/local/lib/libjemalloc.so.2
COPY --from=builder /usr/local/bin/readsb* /usr/local/bin/
COPY --from=builder /usr/local/bin/viewadsb* /usr/local/bin/viewadsb
